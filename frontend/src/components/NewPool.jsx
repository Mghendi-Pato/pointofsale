...
const NewPool = ({ showAddPool, setShowAddPool }) => {
  const [isSmallScreen, setIsSmallScreen] = useState(false);
  const [registerPoolLoading, setRegisterPoolLoading] = useState(false);
  ...
  const useRegisterPool = () => {
    return useMutation(
      ({ poolData, token }) => registerNewPool(poolData, token),
      {
        onMutate: () => {
          setRegisterPoolLoading(true);
        },
        onSuccess: () => {
          setRegisterPoolLoading(false);
        ...
        onError: (error) => {
          setRegisterPoolLoading(false);
          toast.error(error.message || "Failed to register pool");
        },
      }
    );
  };

  const registerPoolMutation = useRegisterPool();
  ...
  const formik = useFormik({
    ...
    validationSchema,
    onSubmit: (values) => {
      registerPoolMutation.mutate({ token, poolData: values });
    },
  });
        ...
        <motion.div
          ...
          <div className="w-full">
            <div className="w-full text-center text-lg py-2">
              <p className="font-roboto font-bold">Add Pool</p>
            </div>
            <form
              ...
              <div className="flex flex-row-reverse justify-between items-center">
                <button
                  ...
                  {registerPoolLoading ? "Adding Pool ..." : "Add Pool"}
                </button>
              </div>
            </form>
          </div>
